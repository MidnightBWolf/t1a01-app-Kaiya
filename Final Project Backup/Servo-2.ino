//T1A06-flex-servo-name.ino

//PUT YOUR VARIABLES HERE

Servo myServoA4;
int   myPosA0, myPosAdjusted;
bool  myWebIsOff=false;
...
void loop(){
	...
	if (myWebIsOff) {    
    	myPosA0 = analogRead(A0);
    	myPosAdjusted = (myPosA0-400)/4;
    	if (myPosAdjusted < 0) {myPosAdjusted = 0;}
    	if (myPosAdjusted > 180) {myPosAdjusted = 180;}
    	myServoA4.write(myPosAdjusted);
    	delay(20);
	}
...
int myMain(String myCode) {
	...
	if (myActivity == "SERV"){	//Servo send
    	myServoA4.write(mySetWrite);
    	return mySetWrite;
	}  
	...
	if (myActivity == "FLEX"){
    	myWebIsOff = mySetWrite;  //where 1 = true 0 = false
    	return mySetWrite;
	}   
